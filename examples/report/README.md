...
AIQ tool that makes use of an Object Store to retrieve data.
...
For the up-to-date installation instructions of MinIO, see [MinIO Github Page](https://github.com/minio/minio) and MinIO client see [MinIO Client Github Page](https://github.com/minio/mc)
...
### Example 1
```
aiq run --config_file examples/report/configs/config.yml --input "Give me the latest report of user 67890"
```

**Expected Output**
```console
aiq run --config_file examples/report/configs/config.yml --input "Give me the latest report of user 67890"
2025-04-23 17:30:04,742 - aiq.cli.commands.start - INFO - Starting AgentIQ from config file: 'examples/report/configs/config.yml'
...
### Example 2
```
aiq run --config_file examples/report/configs/config.yml --input "Give me the latest report of user 12345 on April 15th 2025"
```

**Expected Output**
```console
aiq run --config_file examples/report/configs/config.yml --input "Give me the latest report of user 12345 on April 15th 2025"
2025-04-23 17:35:27,582 - aiq.cli.commands.start - INFO - Starting AgentIQ from config file: 'examples/report/configs/config.yml'
...
### Example 3
```
aiq run --config_file examples/report/configs/config.yml --input 'Create a latest report for user 6789 with the following JSON contents:
    {
        "recommendations": [
            "Update graphics driver",
            "Check for overheating hardware",
...
'
2025-04-24 09:57:16,849 - aiq.cli.commands.start - INFO - Starting AgentIQ from config file: 'examples/report/configs/config.yml'
...
```
aiq run --config_file examples/report/configs/config.yml --input 'Get the latest report for user 6789'
```

**Expected Output**
```console
aiq run --config_file examples/report/configs/config.yml --input 'Get the latest report for user 6789'
2025-04-24 10:00:08,498 - aiq.cli.commands.start - INFO - Starting AgentIQ from config file: 'examples/report/configs/config.yml'

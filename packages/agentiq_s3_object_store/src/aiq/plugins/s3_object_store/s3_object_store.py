...
class S3ObjectStore(ObjectStore):
    def __init__(self, bucket_name: str, access_key: str | None, secret_key: str | None, region: str | None, endpoint_url: str | None):
        self.bucket_name = bucket_name
        self.session = aioboto3.Session()
        access_key = access_key or os.environ.get("OBJECT_STORE_ACCESS_KEY")
        if not access_key:
            raise ValueError("Access key is not set. Please specify it in the environment variable 'OBJECT_STORE_ACCESS_KEY_ID'.")

        secret_key = secret_key or os.environ.get("OBJECT_STORE_SECRET_KEY")
        if not secret_key:
            raise ValueError("Secret key is not set. Please specify it in the environment variable 'OBJECT_STORE_SECRET_ACCESS_KEY'.")
